...
저장소는 :mod:`jobhelper.storage` 가 고른다. 로컬은 SQLite, ``DATABASE_URL`` 이
설정되면 PostgreSQL. 기존 버전의 ``scrapped_jobs`` 테이블을 그대로 이어받아
컬럼만 덧붙이므로 예전 jobs.db가 있어도 데이터를 잃지 않는다.
"""
...
from .storage import (
    SQLITE_PATH as DB_PATH,  # noqa: F401  (하위 호환용 재수출)
...
def init_db(db_path: str | None = None) -> None:
    """테이블을 만들고, 구버전 스키마라면 부족한 컬럼을 채워 넣는다."""
    with connect(db_path) as conn:
        ...
        for name, ddl in _EXTRA_COLUMNS:
            add_column_if_missing(conn, "scrapped_jobs", name, ddl)
...
def save_job(job: dict[str, Any], db_path: str | None = None) -> bool:
    ...
    with connect(db_path) as conn:
        cur = conn.execute(
            insert_or_ignore("scrapped_jobs", _JOB_COLUMNS, ["company", "position"]),
            (
                job.get("source", ""),
                job.get("company", ""),
                job.get("position", ""),
...
def load_jobs(status: str | None = None, db_path: str | None = None) -> list[dict[str, Any]]:
    with connect(db_path) as conn:
        if status:
            ...
            cur = conn.execute("SELECT * FROM scrapped_jobs ORDER BY id DESC")
...
def mark_seen(job_keys: Iterable[str], db_path: str | None = None) -> set[str]:
    ...
    with connect(db_path) as conn:
        ...
        if fresh:
            stamp = now_iso()
            conn.executemany(
                insert_or_ignore("seen_jobs", ["job_key", "first_seen"], ["job_key"]),
